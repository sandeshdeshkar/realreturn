...
const pct = (n: number) =>
  !isFinite(n) || isNaN(n) ? '0%' : (n >= 0 ? '+' : '') + n.toFixed(1) + '%'
const fisher = (nom: number, inf: number) =>
  ((1 + nom / 100) / (1 + inf / 100) - 1) * 100
...
export default function Calculator() {
  ...
  // ── adjustments — both ON by default
  const [inclTax, setInclTax]   = useState(true)
  const [inclInf, setInclInf]   = useState(true)

  // ── tax slab steps
  const TAX_STEPS = [0, 5, 20, 30]
  const taxIndex  = TAX_STEPS.indexOf(tax) === -1 ? 3 : TAX_STEPS.indexOf(tax)

  // ── Calculations (unchanged logic from original) ──────────────────────────
  const calcFD = useCallback((): Res => {
    const r = fdRate / 100
    const tR = inclTax ? tax / 100 : 0
    const iR = inclInf ? inf / 100 : 0
  ...
  }, [tab, lumpsum, monthly, fdRate, years, tax, inf, inclTax, inclInf])
  ...
  }, [tab, lumpsum, monthly, rdRate, years, tax, inf, inclTax, inclInf])
  ...
  const calcMF = useCallback((): Res => {
    ...
    if (tab === 'lumpsum') {
      ...
      // Verified SIP formula — matches Groww, ET Money, AMFI exactly
  ...
  }, [tab, lumpsum, monthly, mfRate, years, tax, inf, inclTax, inclInf])

  const fd  = calcFD()
  const rd  = calcRD()
  const mf  = calcMF()

  const invested = tab === 'lumpsum' ? lumpsum : monthly * years * 12

  // ── winner
  const instruments = [
    { key: 'fd'  as const, name: tab === 'monthly' ? 'Monthly FD' : 'Fixed Deposit', shortName: 'FD',  d: fd,  taxNote: `${tax}% slab tax` },
    { key: 'rd'  as const, name: 'Recurring Deposit',                                shortName: 'RD',  d: rd,  taxNote: `${tax}% slab tax` },
    { key: 'sip' as const, name: 'SIP — Mutual Fund',                                shortName: 'MF',  d: mf,  taxNote: '12.5% LTCG' },
  ]
  const sorted   = [...instruments].sort((a, b) => b.d.postTax - a.d.postTax)
  const winnerKey = sorted[0].key
  const maxCorpus = Math.max(fd.corpus, rd.corpus, mf.corpus)

  // insight chip
  const mfOverFd  = mf.postTax / fd.postTax
  const chipActive = inclTax || inclInf
  const chipText  = chipActive
  ...
  const Slider = ({
    label, val, set, min, max, step, disp, hint,
  }: {
    ...
    <div style={{ marginBottom: '18px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '6px' }}>
        ...
        <span style={{
          ...
          background: '#f1f5f9', padding: '2px 10px', borderRadius: '6px',
        }}>{disp}</span>
      </div>
      <input
  ...
  // ── Result card
  const ResultCard = ({ k }: { k: 'fd' | 'rd' | 'sip' }) => {
    const item   = instruments.find(i => i.key === k)!
    const c      = COLOR[k]
    const isWin  = k === winnerKey
    const estRet = item.d.postTax - item.d.invested
    const pctRet = item.d.invested > 0 ? (estRet / item.d.invested) * 100 : 0

    return (
      <div style={{
        background: '#fff',
        border: isWin ? `2px solid ${c.accent}` : '0.5px solid #e2e8f0',
        borderRadius: '14px',
        padding: '14px 12px',
        position: 'relative',
        flex: 1,
        ...
        {isWin && (
          <div style={{
            ...
            borderRadius: '999px', whiteSpace: 'nowrap', letterSpacing: '0.04em',
          }}>Best returns</div>
        )}
        {/* label */}
        <div style={{ fontSize: '10px', fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase', color: c.label, marginBottom: '6px' }}>
          {item.shortName}
        </div>
        {/* hero gain */}
        <div style={{ fontSize: '20px', fontWeight: 700, color: '#0f172a', fontFamily: 'DM Mono, monospace', lineHeight: 1.1 }}>
          {fmtShort(estRet)}
        </div>
        <div style={{ fontSize: '11px', color: '#94a3b8', marginTop: '2px', marginBottom: '10px' }}>
          {inclTax || inclInf ? 'post-tax returns' : 'est. returns'}
        </div>
        {/* divider */}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '3px' }}>
          ...
          <span style={{ fontSize: '11px', fontWeight: 600, fontFamily: 'DM Mono, monospace', color: '#0f172a' }}>{fmtShort(item.d.postTax)}</span>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
          <span style={{ fontSize: '10px', color: '#94a3b8' }}>Invested</span>
          <span style={{ fontSize: '11px', fontWeight: 600, fontFamily: 'DM Mono, monospace', color: '#0f172a' }}>{fmtShort(item.d.invested)}</span>
        </div>
        {/* pill */}
        <div style={{
  ...
  // ── Bar chart row
  const BarRow = ({ k, maxVal }: { k: 'fd' | 'rd' | 'sip'; maxVal: number }) => {
    const item = instruments.find(i => i.key === k)!
    const c    = COLOR[k]
    const total = item.d.postTax
    const inv   = item.d.invested
    ...
    return (
      <div style={{ marginBottom: '12px' }}>
        ...
        <div style={{ height: '24px', borderRadius: '6px', display: 'flex', overflow: 'hidden', border: '0.5px solid #f1f5f9' }}>
          <div style={{
            width: `${invW}%`, background: c.light,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            fontSize: '9px', fontWeight: 600, color: c.text,
            minWidth: invW > 15 ? 0 : 'auto', overflow: 'hidden',
          }}>
            {invW > 18 ? `${fmtShort(inv)} invested` : ''}
          </div>
          <div style={{
            width: `${gainW}%`, background: c.dark,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            fontSize: '9px', fontWeight: 600, color: '#fff',
            overflow: 'hidden',
          }}>
            {gainW > 14 ? `${fmtShort(gain)} returns` : ''}
          </div>
        </div>
      </div>
    )
  ...
  // ── Tax slab stepped slider
  const TaxSlabSlider = () => {
    const steps = TAX_STEPS
    const curr  = taxIndex
    const trackFill = (curr / (steps.length - 1)) * 100

    return (
      <div style={{ padding: '12px 14px 14px', background: '#f8fafc', borderTop: '0.5px solid #e2e8f0' }}>
        {/* header */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '12px' }}>
          <span style={{ fontSize: '12px', color: '#374151' }}>Your income tax slab</span>
          <span style={{ fontSize: '18px', fontWeight: 700, color: '#1a6b3c', fontFamily: 'DM Mono, monospace' }}>{tax}%</span>
        </div>
        {/* slider */}
        <div style={{ position: 'relative', paddingBottom: '22px' }}>
          <input
            ...
            min={0} max={3} step={1}
            value={taxIndex}
            onChange={e => setTax(TAX_STEPS[+e.target.value])}
            style={{ width: '100%', accentColor: '#1a6b3c', cursor: 'pointer' }}
          />
          {/* stop labels */}
          <div style={{ display: 'flex', justifyContent: 'space-between', position: 'absolute', bottom: '0', left: '0', right: '0' }}>
            {steps.map((s, i) => (
              <div key={s} style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2px' }}>
                <span style={{
                  fontSize: '11px', fontFamily: 'DM Mono, monospace',
                  fontWeight: i === curr ? 700 : 400,
                  color: i === curr ? '#1a6b3c' : '#94a3b8',
                }}>{s}%</span>
              </div>
            ))}
          </div>
        ...
        <div style={{
          ...
          <span style={{ fontSize: '11px', color: '#3C3489', lineHeight: 1.5 }}>
            SIP (MF) is taxed at 12.5% LTCG — not at your income slab rate
          </span>
        </div>
      </div>
    )
  ...
  // ── Toggle row with optional expander
  const AdjustmentRow = ({
    label, sub, on, onToggle, children,
  }: {
    label: string; sub: string; on: boolean; onToggle: () => void; children?: React.ReactNode
  }) => (
    <div style={{ background: '#fff', borderRadius: '12px', border: '0.5px solid #e2e8f0', overflow: 'hidden', marginBottom: '8px' }}>
      <div
        style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '13px 16px', cursor: 'pointer', background: on ? '#f8fafc' : '#fff' }}
        onClick={onToggle}
      >
        <div>
          <div style={{ fontSize: '13px', fontWeight: 500, color: '#0f172a' }}>{label}</div>
          <div style={{ fontSize: '11px', color: on ? '#1a6b3c' : '#94a3b8', marginTop: '2px' }}>{sub}</div>
        </div>
  ...
  // ── Insight chip
  const chipBg    = (inclTax || inclInf) ? '#FAEEDA' : '#EEEDFE'
  const chipDot   = (inclTax || inclInf) ? '#854F0B' : '#534AB7'
  const chipColor = (inclTax || inclInf) ? '#633806' : '#3C3489'
    ...
    <div style={{ minHeight: '100vh', background: '#f8fafc', fontFamily: 'Sora, sans-serif' }}>
      ...
      <div style={{ maxWidth: '560px', margin: '0 auto', padding: '24px 16px 80px' }}>
        ...
        <div style={{ background: '#fff', border: '0.5px solid #e2e8f0', borderRadius: '14px', padding: '20px', marginBottom: '12px' }}>
          ...
          {tab === 'monthly' ? (
            <Slider label="Monthly amount" val={monthly} set={setMonthly}
              min={1000} max={100000} step={500}
              disp={fmt(monthly) + '/mo'} />
          ) : (
            <Slider label="Lumpsum amount" val={lumpsum} set={setLumpsum}
              min={10000} max={5000000} step={10000}
              disp={fmt(lumpsum)} />
          )}

          <Slider label="Time period" val={years} set={setYears}
            min={1} max={30} step={1}
          ...
          {tab === 'monthly' ? (
            <>
              <Slider label="FD rate" val={fdRate} set={setFdRate}
                min={4} max={10} step={0.25} disp={`${fdRate}%`} />
              <Slider label="RD rate" val={rdRate} set={setRdRate}
                min={4} max={9} step={0.25} disp={`${rdRate}%`} />
              <Slider label="SIP expected CAGR" val={mfRate} set={setMfRate}
                min={6} max={20} step={0.5} disp={`${mfRate}%`}
                hint="Nifty 50 delivered ~13% CAGR over 20 years" />
            </>
          ) : (
            <>
              <Slider label="FD rate" val={fdRate} set={setFdRate}
                min={4} max={10} step={0.25} disp={`${fdRate}%`} />
              <Slider label="MF expected CAGR" val={mfRate} set={setMfRate}
                min={6} max={20} step={0.5} disp={`${mfRate}%`}
                hint="Nifty 50 delivered ~13% CAGR over 20 years" />
            </>
          )}
        ...
        <div style={{ marginBottom: '12px' }}>
          ...
          <AdjustmentRow
            ...
            sub={inclInf ? `6% p.a. · ON` : '6% p.a.'}
            on={inclInf}
            onToggle={() => setInclInf(v => !v)}
          />

          {/* Tax toggle + slab slider */}
          <AdjustmentRow
            label="Show post-tax returns"
            sub={inclTax ? `${tax}% slab · ON` : 'Slide to pick your tax slab'}
            on={inclTax}
            onToggle={() => setInclTax(v => !v)}
          >
            <TaxSlabSlider />
          </AdjustmentRow>
        </div>
        ...
        <div style={{
          ...
          background: chipBg, borderRadius: '10px',
          padding: '10px 14px', marginBottom: '16px',
        }}>
          <div style={{ width: '7px', height: '7px', borderRadius: '50%', background: chipDot, flexShrink: 0 }} />
          <span style={{ fontSize: '13px', fontWeight: 500, color: chipColor, lineHeight: 1.4 }}>
            {chipText}
          </span>
        </div>
        ...
        <div style={{ marginBottom: '8px' }}>
          <div style={{ fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#94a3b8', marginBottom: '10px' }}>
            {inclTax || inclInf ? 'Post-tax returns' : 'Est. returns'}
          </div>
        ...
        {/* ── Bar chart ───────────────────────────────────────────── */}
        <div style={{ background: '#f8fafc', borderRadius: '14px', padding: '16px', marginTop: '16px', marginBottom: '16px', border: '0.5px solid #e2e8f0' }}>
          <div style={{ fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#94a3b8', marginBottom: '14px' }}>
            Return breakdown
          </div>
          {tab === 'monthly' ? (
            <>
              <BarRow k="fd"  maxVal={maxCorpus} />
              <BarRow k="rd"  maxVal={maxCorpus} />
              <BarRow k="sip" maxVal={maxCorpus} />
            </>
          ) : (
            <>
              <BarRow k="fd"  maxVal={maxCorpus} />
          ...
          <div style={{ marginTop: '10px', fontSize: '11px', color: '#94a3b8' }}>
            Light = invested · Dark = {inclTax || inclInf ? 'post-tax returns' : 'est. returns'}
          </div>
        </div>
        ...
        <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: '40px', marginBottom: '40px' }}>
          ...
          {[
            ...
            },
            {
              ...
              a: 'As per Union Budget 2024, equity mutual fund gains held for more than 12 months are classified as Long Term Capital Gains (LTCG). The tax rate is 12.5% on gains above ₹1.25 lakh per financial year. This makes equity mutual funds significantly more tax-efficient than FDs for investors in the 20% or 30% slab.',
            },
            {
              q: 'Is RD better than FD for monthly savings?',
              a: 'For monthly savings, RD is a better structure than FD because it is designed for monthly contributions and compounds quarterly (standard Indian bank formula). However, both are taxed at your income slab rate on the interest earned, so the net tax drag is similar.',
            },
            {
              q: 'What FD rate beats inflation after tax?',
              a: 'For an investor in the 30% tax slab with 6% inflation, an FD would need to offer approximately 8.6% interest to deliver a zero real return. Most Indian banks offer 6.5–7.5%, which means FD investors in the highest tax bracket are consistently losing purchasing power.',
            },
            {
              q: 'How long should I stay invested in mutual funds to beat FD?',
              a: 'Equity mutual funds typically begin outperforming FDs in real terms from Year 3–5 onwards. Over 10–15 year periods, the combination of higher CAGR and lower LTCG tax makes equity mutual funds significantly superior to FDs for long-term wealth creation.',
            },
          ].map((faq, i) => (
